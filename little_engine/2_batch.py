...
def main(num_threads):
    ...
    sm.use_technique(BatchThreading(threads=num_threads, task_multiplier=4, pipeline=False))
    with profile("simgr run", phases):
        sm.run()
...
if __name__ == "__main__":
    ...
    print(f"Using {num_threads} / {os.cpu_count()} threads")
    before = time.time()
    res, phases = main(num_threads)
    after = time.time()
    print(res)
